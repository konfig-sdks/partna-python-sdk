...
from partna_python_sdk.exceptions_base import ApiValueError
from partna_python_sdk.exceptions import ClientConfigurationError
from partna_python_sdk.exceptions import InvalidHostConfigurationError
...
conf = partna_python_sdk.Configuration(
    ...
    def __init__(self, host=None,
        ...
        """Constructor
        """
        self.host = "https://staging-biz.coinprofile.co/v2" if host is None else host
        """Default Base url
        """
        ...
        self.api_key = {}
        if admin_secret:
            self.api_key['AdminSecret'] = admin_secret
        if api_key:
            self.api_key['ApiKey'] = api_key
        if api_user:
            self.api_key['ApiUser'] = api_user
        if user_version:
            self.api_key['UserVersion'] = user_version
        ...
        """Logging Settings
        """
        self.logger = logging.getLogger("partna_python_sdk")
    ...
    def auth_settings(self):
        ...
        """
        auth = {}
        if 'AdminSecret' in self.api_key:
            auth['AdminSecret'] = {
                'type': 'api_key',
                'in': 'header',
                'key': 'X-Admin-Secret',
        ...
        if 'UserVersion' in self.api_key:
            auth['UserVersion'] = {
                'type': 'api_key',
                'in': 'header',
                'key': 'X-User-Version',
